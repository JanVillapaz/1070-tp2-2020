...
# create user 'etudiant' with the password 'INF1070pass'

RUN apt-get update && apt-get install -y\
    sudo openssl

RUN useradd -m -g adm -G sudo -d /home/etudiant -s /bin/bash -p `(echo "INF1070pass" |openssl passwd -1 -stdin)` etudiant
